        ...
        for alpha_scale in [0.1,0.075,0.05,0.025,0.01]:
            ...
            for comm_name, community in community_data_lists.items():
                ...
                if comm_name == "-1":
                    ...
                    print("Community Rank",modules_community_dataset["community"][idx][comm_name]["dataset"]["all"], flush=True)
                    module_list =  community["modules"]
                    reset_model= randomize_model(model, community["modules"], alpha_scale)
                ...
                for dataset_name_label,dataset_name in zip(module_dataset, module_dataset_info_format):
                    if comm_name == "-1":
                        ...
                        rank = modules_community_dataset["community"][idx][comm_name]["dataset"]["all"].index(dataset_name_label)
                    args_dataset = Namespace(save_data = "",do_train_both = False,nsamples=10,seqlen=500,model_type="llama",num_process=10,max_length=0,device='cuda',fine_tune=False,evaluation_size=20, seed=0)
                    ...
                    module_accuracy.append(accuracy[2])
