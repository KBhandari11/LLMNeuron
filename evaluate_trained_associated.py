...
def train_validation_dataloader(train_dataloader_list, validation_size):
    tensor_data = torch.stack([x.squeeze(0) for x,_,_ in train_dataloader_list[0:(-validation_size+1)]],dim=0) # transform to torch tensor
    tensor_atten = torch.stack([y.squeeze(0) for _,y,_ in train_dataloader_list[0:(-validation_size+1)]],dim=0) # transform to torch tensor
    tensor_label = torch.stack([z.squeeze(0) for _,_,z in train_dataloader_list[0:(-validation_size+1)]],dim=0)
    train_dataset = TensorDataset(tensor_data,tensor_atten,tensor_label) 

    val_tensor_data = torch.stack([x.squeeze(0) for x,_,_ in train_dataloader_list[(-validation_size+1):]],dim=0) # transform to torch tensor
    val_tensor_atten = torch.stack([y.squeeze(0) for _,y,_ in train_dataloader_list[(-validation_size+1):]],dim=0) # transform to torch tensor
    val_tensor_label = torch.stack([z.squeeze(0) for _,_,z in train_dataloader_list[(-validation_size+1):]],dim=0)
    val_dataset = TensorDataset(val_tensor_data,val_tensor_atten,val_tensor_label) 
...
def test_multigpu(model,tokenizer,rank, world_size, test_loader):
    ...
    with torch.no_grad():
        for data,atten, target in test_loader:
...
def fsdp_main(rank, world_size, dataset,val_dataset, model, tokenizer, total_epoch,evaluation_parameters,save_result_parameters,save_checkpoint,lr=1e-5):
    ...
    if rank == 0:
        ...
        accuracy, _ = evaluate(model=model,tokenizer=tokenizer,testloader=test_dataset,args=evaluation_args)
        print(accuracy[2],accuracy[0],accuracy[1],loss_list,compute_l2_norm(model),flush=True)
        ...
        torch.save(states, f"/data/Kushal/MLNeuron/checkpoint/{save_checkpoint}.pt") 
    free_mem(model)
    cleanup()
...
def save_result_csv(filename, data):
    with open(filename, mode='a', newline='') as file:
        ...
        writer.writerow({key: data[key] for key in data.keys()})
...
if __name__ == "__main__":
    ...
    for idx, model_name in enumerate(modules_community_dataset["model"]):
        ...
        for comm_name, community in community_data_lists.items(): 
            if len(community["modules"]) < 10:
                continue
            if int(comm_name) not in [0,1]:
                continue
            ...
            for rank, (dataset_name_label,dataset_name)  in enumerate(zip(module_dataset_kl, module_dataset_info_format_kl)):
                if dataset_name_label == "which_wiki_edit":
                    continue
                ...
                ''' 
                module_accuracy_run = []
                for run_name in ["Random","Community","All"]:#["Community","Random","All"]:
                    ...
                    save_result_parameters = f'./result/randomize_accuracy/randomize_data_new_kl_longer_{sys.argv[2]}.csv', save_dictionary
